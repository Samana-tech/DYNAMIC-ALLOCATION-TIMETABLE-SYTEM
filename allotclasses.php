...
<form action="allotclasses.php" method="post" style="margin-top: 100px">
    ...
    <div align="center">
        <select name="course" class="list-group-item">
            ...
            <option value="1">Computer Science 1st Year</option>
            <option value="2">Computer Science 2nd Year</option>
            <option value="3">Computer Science 3rd Year</option>
            <option value="4">Computer Science 4th Year</option>
        </select>
    </div>
...
$q = mysqli_query(mysqli_connect("localhost", "root", "", "timetabledb"),
    "SELECT * FROM classrooms ");
$courses = array('Computer Science 1st Year', 'Computer Science 2nd Year', 'Computer Science 3rd Year', 'Computer Science 4th Year');
...
while ($row = mysqli_fetch_assoc($q)) {
    ...
    // Calculate the index for the courses array
    $courseIndex = $row['status'] - 2;

    // Check if the index is valid
    if ($courseIndex >= 0 && $courseIndex < count($courses)) {
        echo "<tr><td>{$row['name']}</td>
                <td>{$courses[$courseIndex]}</td>
                <td><button>Delete</button></td>
                </tr>\n";
    } else {
        // Handle the case where the status is unexpected
        echo "<tr><td>{$row['name']}</td>
                <td>Unknown</td>
                <td><button>Delete</button></td>
                </tr>\n";
