...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Redirect to fetch empty rooms
    header("Location: show_rooms.php?unit_code=$unit_code&course_group_codes=" . implode(',', $course_group_codes) . "&day=$day&lesson=$lesson");
    exit();
}
?>
<!DOCTYPE html>
    ...
    <div class="container" style="margin-top: 70px;">
        <div class="row">
            <div class="col-md-6 col-md-offset-3">
                <h2 class="custom-caption">Book Make-Up Class</h2>
                <form method="POST" action="makeup.php" class="scrollable-form">
                    <div class="form-group">
                        <label for="unit_code" class="custom-caption">Unit Code:</label>
                        <select name="unit_code" class="form-control" required>
                            <?php
                            $units = mysqli_query($conn, "SELECT unit_code FROM units");
                    ...
                    <div class="form-group">
                        <label for="course_group_code" class="custom-caption">Course Group Code:</label><br>
                        <?php
                        $course_groups = mysqli_query($conn, "SELECT course_group_code FROM course_groups");
                        while ($row = mysqli_fetch_assoc($course_groups)) {
                            echo "<label class='checkbox-inline' style='color: white;'><input type='checkbox' name='course_group_code[]' value='{$row['course_group_code']}'> {$row['course_group_code']}</label>";
                        }
                        ?>
                    </div>

                    <div class="form-group">
                        <label for="day"class="custom-caption">Day:</label>
                        <select name="day" class="form-control" required>
                            <?php
                            $days = mysqli_query($conn, "SELECT DISTINCT day FROM timeslots");
                    ...
                    <div class="form-group">
                        <label for="lesson" class="custom-caption">Lesson (Time Slot):</label>
                        <select name="lesson" class="form-control" required>
                            <?php
                            $lessons = mysqli_query($conn, "SELECT start_time, end_time FROM timeslots");
                            while ($row = mysqli_fetch_assoc($lessons)) {
                                $lesson = $row['start_time'] . ' - ' . $row['end_time'];
                    ...
                    <button type="submit" class="btn btn-primary">Fetch Available Rooms</button>
                </form>
            </div>
        </div>
    </div>
