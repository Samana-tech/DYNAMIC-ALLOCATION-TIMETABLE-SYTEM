        ...
        <caption>
            <strong><br><br>
                ...
                if (isset($_POST['select_semester'])) {
                    ...
                    $r = mysqli_fetch_assoc(mysqli_query(mysqli_connect("localhost", "root", "", "timetabledb"), "SELECT * from classrooms
                                WHERE status = '$year'"));
                    echo " ( " . $r['name'], " ) ";
                }
                ?>
        ...
        <tr>
            ...
            if (isset($_POST['select_semester']) && $_POST['select_semester'] % 2 !== 0) {
                $q = mysqli_query(mysqli_connect("localhost", "root", "", "timetabledb"),
                    "SELECT * FROM" . $table);
                $qq = mysqli_query(mysqli_connect("localhost", "root", "", "timetabledb"),
                    ...
                    while ($r = mysqli_fetch_assoc($qq)) {
                        if ($r['isAlloted'] == 1 && $r['semester'] == $_POST['select_semester']) {
                            ...
                            if (isset($r['allotedto'])) {
                                $id = $r['allotedto'];
                                $qqq = mysqli_query(mysqli_connect("localhost", "root", "", "timetabledb"),
...
<script type="text/javascript">
    function gendf() {
        ...
        doc.addHTML(document.getElementById('TT'), function () {
            doc.save('<?php echo "timetabledb semester " . $_POST["select_semester"]?>' + '.pdf');
            alert("Downloaded!");
        });
    }
</script>
